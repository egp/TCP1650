...
static void testGetButtonsWhileOffRestoresOffStateWithoutDisplayWrites() {
  ...
  TEST_ASSERT_EQ(static_cast<std::size_t>(9u), transport.ops.size());
  ...
  TEST_ASSERT_EQ(static_cast<uint8_t>(tcp1650EncodeHexDigit(2u) | TCP1650_DOT_BIT),
                 device.segmentAt(1u));
...
static void testGetButtonsReadFailureWhileOffRestoresOffStateWithoutDisplayWrites() {
  FaultyTransport transport;
  TCP1650_Device device(transport);
  ...
  TEST_ASSERT_EQ(tcp1650EncodeDigit(2u), device.segmentAt(0u));
  TEST_ASSERT_EQ(tcp1650EncodeDigit(4u), device.segmentAt(1u));
  TEST_ASSERT_EQ(tcp1650EncodeDigit(6u), device.segmentAt(2u));
  TEST_ASSERT_EQ(static_cast<uint8_t>(tcp1650EncodeDigit(8u) | TCP1650_DOT_BIT),
                 device.segmentAt(3u));
}

static void testGetButtonsEntryFailureWhileOffAttemptsRestoreWithoutDisplayWrites() {
  FaultyTransport transport;
  TCP1650_Device device(transport);
  ...
  const uint8_t buttons = device.getButtons();
  TEST_ASSERT_EQ(static_cast<uint8_t>(0u), buttons);
  TEST_ASSERT_EQ(static_cast<std::size_t>(7u), transport.ops.size());
  ...
  TEST_ASSERT_EQ(static_cast<uint8_t>(tcp1650EncodeHexDigit(3u) | TCP1650_DOT_BIT),
                 device.segmentAt(2u));
...
void runOffStateButtonReadTests() {
  testGetButtonsWhileOffRestoresOffStateWithoutDisplayWrites();
  testGetButtonsReadFailureWhileOffRestoresOffStateWithoutDisplayWrites();
  testGetButtonsEntryFailureWhileOffAttemptsRestoreWithoutDisplayWrites();
}